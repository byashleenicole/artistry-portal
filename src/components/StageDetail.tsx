...
const styles: Record<string, CSSProperties> = {
  ...
  sectionLabel: { fontSize: '11px', fontWeight: '500', letterSpacing: '0.07em', textTransform: 'uppercase' as const, color: '#888780', marginBottom: '12px', marginTop: '32px' },
  ...
  downloadBtn: { display: 'inline-block', marginTop: '12px', fontSize: '12px', padding: '7px 14px', borderRadius: '8px', border: '0.5px solid #d3d1c7', color: '#2c2c2a', textDecoration: 'none', backgroundColor: '#faf9f7' },
  divider: { height: '0.5px', backgroundColor: '#f1efea', margin: '16px 0' },
  commentLabel: { fontSize: '11px', fontWeight: '500', textTransform: 'uppercase' as const, color: '#888780', marginBottom: '10px' },
  ...
  revTag: { fontSize: '10px', padding: '2px 7px', borderRadius: '10px', backgroundColor: '#faeeda', color: '#854f0b', fontWeight: '500' },
  resTag: { fontSize: '10px', padding: '2px 7px', borderRadius: '10px', backgroundColor: '#e1f5ee', color: '#0f6e56', fontWeight: '500' },
  ...
  revBtn: { fontSize: '12px', padding: '7px 14px', borderRadius: '8px', border: '0.5px solid #e8c97a', backgroundColor: '#faeeda', color: '#854f0b', cursor: 'pointer' },
    ...
    <div style={styles.assetCard}>
      <div style={styles.assetHeader}>
        <div>
          ...
          <div style={styles.assetMeta}>{asset.version} · Uploaded {new Date(asset.uploaded_at).toLocaleDateString()}</div>
        </div>
        <span style={styles.fileTag}>{asset.file_type || 'FILE'}</span>
      </div>
      {asset.drive_url && (
        <a href={asset.drive_url} target="_blank" rel="noopener noreferrer" style={styles.downloadBtn}>
          View / download
        </a>
      )}
      ...
      {comments.map(comment => (
        <div key={comment.id} style={styles.commentItem}>
          ...
          <div style={styles.commentMeta}>
            ...
            {comment.type === 'revision_request' && <span style={styles.revTag}>Revision request</span>}
            {comment.resolved
              ? <span style={styles.resTag}>Resolved</span>
              : <span style={{ cursor: 'pointer', textDecoration: 'underline', fontSize: '11px', color: '#888780' }} onClick={() => toggleResolved(comment.id, comment.resolved)}>Mark resolved</span>
            }
          </div>
        </div>
      ))}
      ...
      <div style={styles.commentActions}>
        <button style={styles.noteBtn} disabled={submitting || !body.trim()} onClick={() => submitComment('note')}>Add note</button>
        <button style={styles.revBtn} disabled={submitting || !body.trim()} onClick={() => submitComment('revision_request')}>Request revision</button>
      </div>
    </div>
  )
}

export default function StageDetail({ stage, clientId, clientName, onBack, onApproved, onSchedule, onProfile }: Props) {
    ...
    <div style={styles.page}>
      ...
      <div style={styles.inner}>
        ...
        <div style={styles.stageMeta}>
          {approved
            ? `Approved ${stage.approved_at ? new Date(stage.approved_at).toLocaleDateString() : 'recently'}`
        ...
        {!loading && assets.length === 0 && <div style={styles.emptyAssets}>No assets uploaded yet. Check back soon!</div>}
        {assets.map(asset => <AssetCard key={asset.id} asset={asset} clientId={clientId} />)}
        {!approved && !loading && (
          <div style={styles.approveSection}>
            <div style={styles.approveHeading}>Ready to move forward?</div>
            <div style={styles.approveSub}>Approving this stage confirms you are happy with the work and ready for the next phase.</div>
            <button style={styles.approveBtn} onClick={handleApprove} disabled={approving}>
              {approving ? 'Approving...' : 'Approve this stage'}
            </button>
          </div>
        )}
        {approved && <div style={styles.approvedBanner}>This stage has been approved.</div>}
      </div>
    </div>
  )
}